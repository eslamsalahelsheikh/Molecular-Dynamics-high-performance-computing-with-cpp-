...
void Energy::update(Atoms &atoms, double epsilon, double sigma , double mass) {
    ...
    std::cout << "potential energy: " << potential_energy_ << " temperature: " << temperature_ << std::endl;
}
...
void Energy::berendsen_thermostat(Atoms &atoms, double desired_temperature, double timestep, double relaxation_time){
    ...
    double scaling_factor = sqrt (1+ ((temperature_ratio-1) * timestep / relaxation_time));
